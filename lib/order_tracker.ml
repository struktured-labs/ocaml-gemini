...
module T = struct
    ...
    module Order = struct 
      ...
      module T = struct
          type t = [ `Order_response | `Order_event ] [@@deriving sexp, compare, equal, enumerate]
      ...
      let of_order_response (response: Status.response) = 
        let number_of_str_some  = fun x -> Decimal_number.of_string x |> Option.some in
        {
    ...
    let empty_summary = {total_original=0.; total_executed=0.; total_remaining=0.}


    type t = {orders: Order.t Order_map.t;
              summary: summary
    }[@@deriving sexp]
    ...
    let summary (t:t) =
      Map.fold ~f:(fun ~key:_ ~(data:Order.t) {total_original; total_executed; total_remaining} ->
        ...
        let remaining_amount = Option.value event.remaining_amount ~default:0. in
