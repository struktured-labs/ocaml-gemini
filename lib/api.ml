...
module Events = struct
  type t = {
    balance : [ `Ok of Balances.response | Rest.Error.post ] Inf_pipe.Reader.t;
    trades :
    ...
    order_events : [`Ok of Order_events.response | Order_events.Error.t] Pipe.Reader.t;
    market_data : Market_data.event Inf_pipe.Reader.t Symbol.Map.t;
    ledger: Ledger.Entry.t Pipe.Reader.t Symbol.Map.t;
    order_books: [`Ok of Order_book.Book.t | Market_data.Error.t] Pipe.Reader.t Symbol.Map.t
  }

  let create ?(symbols: Symbol.t list = []) ?(order_ids: Order_id.t list = []) cfg nonce : t Deferred.t =
    ...
    Deferred.List.map symbols ~how:`Sequential ~f:(fun symbol ->
        ...
        Pipe.filter_map pipe ~f:(function
          ...
          |  #Market_data.Error.t as e ->
                  begin Log.Global.error_s (Market_data.Error.sexp_of_t e); None end)
        |> return
        >>= fun pipe ->
    ...
    >>= fun market_data ->
    let market_data = Symbol.Map.of_alist_exn (List.zip_exn symbols market_data) in
    Order_book.Books.pipe cfg ~symbols () >>= fun order_books ->
    Order_events.client cfg ~nonce ?query:None ?uri_args:None () >>= fun order_events ->
    let init = Symbol.Map.empty in 
    let order_events, order_events_exn = Pipe.fork ~pushback_uses:`Fast_consumer_only order_events in
    let order_books_fork = Map.map ~f:(Pipe.fork ~pushback_uses:`Fast_consumer_only) order_books in
    let order_books, order_books_exn = Map.map order_books_fork ~f:fst, Map.map order_books_fork ~f:snd in
    let order_books_exn = Map.map ~f:(Pipe.map ~f:Poly_ok.ok_exn') order_books_exn in
    let order_events_exn = Pipe.map ~f:Poly_ok.ok_exn' order_events_exn in
     Ledger.pipe ?num_values:None ?how:None ?behavior:None ~init order_books_exn order_events_exn >>| fun ledger ->
    { balance; trades; order_status; order_events;market_data;order_books;ledger }
