  ...
  module Trade_event = struct
    ...
    module Decorated = struct
      ...
      let create ~event_id ~timestamp ({ tid; price; amount; maker_side } : T.t)
        ...
        | false ->
          failwith "logical error: event_id and trade id (tid) should be equal"
    end
        ...
        | Result.Ok (socket_sequence, message_type) ->
          let json' =
            `Assoc
              ...
              |> fun assoc ->
                List.Assoc.remove ~equal:String.equal assoc "socket_sequence" )
          in
          ...
          | `Update ->
            Update.of_yojson json' |> Result.map ~f:(fun event -> `Update event)
          )
        ...
        ~f:(fun csv_of_events (event : event) ->
          ...
          | `Block_trade block_trade ->
            Csv_of_event.add' csv_of_events `Block_trade
              ...
              [ Block_trade_event.to_decorated ~event_id ~timestamp block_trade
              ] )
end
